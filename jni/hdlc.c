...
static void ax25_disp_packet(struct demod_state *s, unsigned char *bp, unsigned int len)
{
	...
	send_frame_to_java(bp, len);
        if (bp[1] & 1) {
                ...
                fprintf(debug_output, "%s: fm ? to ", s->dem_par->name); fflush(debug_output);
                i = (bp[2] >> 2) & 0x3f;
                if (i) {
			verbprintf(0, "%c",i+0x20);
			fprintf(debug_output, "%c",i+0x20); fflush(debug_output);
                }
			...
			fprintf(debug_output, "%c",i+0x20); fflush(debug_output);
                ...
                fprintf(debug_output, "-%u QSO Nr %u", bp[6] & 0xf, (bp[0] << 6) | (bp[1] >> 2)); fflush(debug_output);
                bp += 7;
                len -= 7;
        } else {
                /*
			...
			return;
                ...
                fprintf(debug_output, "%s: fm ", s->dem_par->name); fflush(debug_output);
		for(i = 7; i < 13; i++) 
                  if ((bp[i] &0xfe) != 0x40) {
                                verbprintf(0, "%c",bp[i] >> 1);
                                fprintf(debug_output, "%c",bp[i] >> 1); fflush(debug_output);
                  }
                verbprintf(0, "-%u to ",(bp[13] >> 1) & 0xf);
                fprintf(debug_output, "-%u to ",(bp[13] >> 1) & 0xf); fflush(debug_output);
                for(i = 0; i < 6; i++) 
                  if ((bp[i] &0xfe) != 0x40) {
                                verbprintf(0, "%c",bp[i] >> 1);
                                fprintf(debug_output, "%c",bp[i] >> 1); fflush(debug_output);
                ...
                fprintf(debug_output, "-%u",(bp[6] >> 1) & 0xf); fflush(debug_output);
                bp += 14;
                len -= 14;
                if ((!(bp[-1] & 1)) && (len >= 7)) {
			verbprintf(0, " via ");
			fprintf(debug_output, " via "); fflush(debug_output);
                }
                while ((!(bp[-1] & 1)) && (len >= 7)) {
                        for(i = 0; i < 6; i++) 
                          if ((bp[i] &0xfe) != 0x40) {
                        ...
                        if ((!(bp[-1] & 1)) && (len >= 7)) {
                                verbprintf(0, ",");
                                fprintf(debug_output, ","); fflush(debug_output);
                        }
                }
        }
        if(!len) 
        ...
        if (!(i & 1)) {
                ...
                fprintf(debug_output, " I%u%u%c",(i >> 5) & 7,(i >> 1) & 7,j); fflush(debug_output);
        } else if (i & 2) {
                /*
                 * U frame
                 */
                ...
                case 0x03:
                        verbprintf(0, " UI%c",j);
                        fprintf(debug_output, " UI%c",j); fflush(debug_output);
                        break;
                case 0x2f:
                        verbprintf(0, " SABM%c",j);
                        fprintf(debug_output, " SABM%c",j); fflush(debug_output);
                        break;
                case 0x43:
                        verbprintf(0, " DISC%c",j);
                        fprintf(debug_output, " DISC%c",j); fflush(debug_output);
                        break;
                case 0x0f:
                        verbprintf(0, " DM%c",j);
                        fprintf(debug_output, " DM%c",j); fflush(debug_output);
                        break;
                case 0x63:
                        verbprintf(0, " UA%c",j);
                        fprintf(debug_output, " UA%c",j); fflush(debug_output);
                        break;
                case 0x87:
                        verbprintf(0, " FRMR%c",j);
                        fprintf(debug_output, " FRMR%c",j); fflush(debug_output);
                        break;
                default:
                        verbprintf(0, " unknown U (0x%x)%c",i & (~0x10),j);
                        fprintf(debug_output, " unknown U (0x%x)%c",i & (~0x10),j); fflush(debug_output);
                        break;
                }
        } else {
                /*
                ...
                case 0x1:
                        ...
                        fprintf(debug_output, " RR%u%c",(i >> 5) & 7,j); fflush(debug_output);
                        break;
                case 0x5:
                        verbprintf(0, " RNR%u%c",(i >> 5) & 7,j);
                        fprintf(debug_output, " RNR%u%c",(i >> 5) & 7,j); fflush(debug_output);
                        break;
                case 0x9:
                        verbprintf(0, " REJ%u%c",(i >> 5) & 7,j);
                        fprintf(debug_output, " REJ%u%c",(i >> 5) & 7,j); fflush(debug_output);
                        break;
                default:
                  verbprintf(0, " unknown S (0x%x)%u%c", i & 0xf, 
                             (i >> 5) & 7, j);
                  fprintf(debug_output, " unknown S (0x%x)%u%c", i & 0xf, 
                          (i >> 5) & 7, j); fflush(debug_output);
                  break;
                }
        }
        if (!len) {
        ...
        while (len) {
                ...
                if ((i >= 32) && (i < 128)) {
                        verbprintf(0, "%c",i);
                        fprintf(debug_output, "%c",i); fflush(debug_output);
                }
                else if (i == 13) {
                  if (j) {
                                verbprintf(0, "\n");
